...
Deno.test("pawn positions equal", () => testMove("A2", "A2", false, false));

Deno.test("pawn black forward 1 from B7", () => testMove("B7", "B6", false, true));

Deno.test("pawn black forward 2 from B7", () => testMove("B7", "B5", false, true));
...
Deno.test("pawn black backwards 1 from B6", () =>
  testMove("B6", "B7", false, false));

Deno.test("pawn black left 1 from B6", () => testMove("B6", "A6", false, false));

Deno.test("pawn black right 1 from B6", () => testMove("B6", "C6", false, false));

// White pieces directions

Deno.test("pawn white forward 1 from B2", () => testMove("B2", "B3", true, true));

Deno.test("pawn white forward 2 from B2", () => testMove("B2", "B4", true, true));

Deno.test("pawn white forward 2 from B3", () => testMove("B3", "B5", true, false));

Deno.test("pawn white backwards 1 from B3", () => testMove("B3", "B2", true, false));

Deno.test("pawn white backwards 2 from B3", () => testMove("B3", "B1", true, false));

// Diagonal takes

Deno.test("pawn black diagonal take front right", () =>
  testCollision("B7", "C6", false, "C6", new Pawn(true), true));

Deno.test("pawn black diagonal take front left", () =>
  testCollision("B7", "A6", false, "A6", new Pawn(true), true));

Deno.test("pawn black diagonal take back right", () =>
  testCollision("B6", "C7", false, "C7", new Pawn(true), false));

Deno.test("pawn black diagonal 2 take front right", () =>
  testCollision("B7", "D5", false, "D5", new Pawn(true), false));

Deno.test("pawn black diagonal take front right same color", () =>
...
Deno.test("pawn black forward 2 collision same color", () =>
  testCollision("B7", "B5", false, "B5", new Pawn(false), false));

Deno.test("pawn white forward 1 collision", () =>
  testCollision("B2", "B3", true, "B3", new Pawn(false), false));

Deno.test("pawn black forward 2 intermediate collision", () =>
  testCollision("C7", "C5", false, "C6", new Pawn(false), false));

Deno.test("pawn white forward 2 intermediate collision opponent", () =>
